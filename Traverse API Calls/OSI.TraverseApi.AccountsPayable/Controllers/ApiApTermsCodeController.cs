...
namespace OSI.TraverseApi.AccountsPayable.Controllers
{
    public class ApiApTermsCodeController : ApiControllerBase
    {
        ...
        {
            PropertyDictionary.Add(TermsCodeBase.Columns.DiscDayOfMonth.ToString(),
                (entity) =>
                {
                    if (!(bool)entity.DiscDayOfMonth)
                        entity.DiscMinDays = new short?(0);
                });

            EntityPropertyDictionary.Add(TermsCodeBase.Columns.DiscMinDays.ToString(),
                (entity, e) =>
                {
                    bool? isDayOfMonth = ApiUserSkipped.IsApiUserSkipped(entity.DiscDayOfMonth) ? false : (bool?)entity.DiscDayOfMonth;

                    if (!(bool)isDayOfMonth)
                        e.Handled = true;
                });
        }
