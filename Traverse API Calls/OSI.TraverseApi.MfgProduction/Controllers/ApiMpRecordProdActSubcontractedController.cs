...
{
    public class ApiMpRecordProdActSubcontractedController : ApiControllerBase
    {
        #region Web Methods
        [ApiRoute(FunctionID, 2f, "order/{orderno}/release/{releaseno:int}/subcontract/{reqid:int}/detail/{id:int?}", typeof(SubContractDtl))]
        ...
        [ApiRoute(FunctionID, 2f, "order/{orderno}/release/{releaseno:int}/subcontract/{reqid:int}/detail", typeof(SubContractDtl))]
        ...
        {
            //Subcontract Detail Property Changes
            PropertyDictionary.Add(SubContractDtlBase.Columns.TransDate.ToString(), TransDatePropertyChanged);
            PropertyDictionary.Add(SubContractDtlBase.Columns.GlPeriod.ToString(), FiscalPeriodPropertyChanged);
            PropertyDictionary.Add(SubContractDtlBase.Columns.FiscalYear.ToString(), FiscalPeriodPropertyChanged);
        }
        #endregion Overrides
        ...
        protected virtual async Task<EntityList<SubContractDtl>> Load(string orderNo, int releaseNo, int reqId, int? id)
        {
            var list = this.CurrentSubContract?.SubContractDtlList as EntityList<SubContractDtl>;

            if (this.CurrentSubContract == null || this.CurrentSubContract.Parent.ReqId != reqId)
            {
                await Load(orderNo, releaseNo, reqId);

                this.CurrentSubContract = this.CurrentRequirement?.SubContractSummary;
                list = this.CurrentSubContract?.SubContractDtlList;
            }
        ...
        protected virtual async Task<SubContractDtl> Find(string orderNo, int releaseNo, int reqId, int id)
        {
            var list = await Load(orderNo, releaseNo, reqId, id);
            return list?.Find(x => x.SeqNo == id);
        }

        protected virtual async Task<List<SubContractDtl>> ProcessEditRequest(bool isCreate, dynamic body, string orderNo, int releaseNo, int reqId, int? id)
        {
            object[] list;
            ...
            var entityList = new List<SubContractDtl>();
        ...
        protected virtual async Task<SubContractDtl> ProcessBodyItem(bool isCreate, dynamic bodyItem, string orderNo, int releaseNo, int reqId, int? id)
        {
            ...
            if (isCreate)
            {
                ...
                if (this.CurrentSubContract?.OrderReleaseStatus == OrderReleaseStatus.InProcess)
                {
                    entity = this.CurrentSubContract?.SubContractDtlList?.AddNew();
                    entity.SetDefaults();
                }
                else
                    throw new InvalidValueException("Subcontract Release Status is invalid.");
            }
            else if (entity == null)
        ...
        protected virtual async Task MarkToDelete(string orderNo, int releaseNo, int reqId, int id)
        {
            ...
            this.CurrentSubContract?.SubContractDtlList?.Remove(entity);
            this.Provider.Update(this.CompId);
        }

        protected virtual void TransDatePropertyChanged(SubContractDtl entity)
        {
        ...
        protected virtual void FiscalPeriodPropertyChanged(SubContractDtl entity)
        {
        ...
        {
            Action<SubContractDtl> action = null;
            if (PropertyDictionary.TryGetValue(e.PropertyName, out action))
                action.Invoke(sender as SubContractDtl);
        }
        ...
        protected SubContractSum CurrentSubContract { get; set; }

        protected SortedDictionary<string, Action<SubContractDtl>> PropertyDictionary { get; } = new SortedDictionary<string, Action<SubContractDtl>>();

        protected SortedDictionary<string, Action<dynamic, ApiEntityPropertyChangingArgs>> EntityPropertyDictionary { get; } = new SortedDictionary<string, Action<dynamic, ApiEntityPropertyChangingArgs>>();
        #endregion Properties 

        #region Fields
        public const string FunctionID = "d205c62b-951e-4a8c-976b-e1a2f6ed6449";
        #endregion Fields
    }
}
