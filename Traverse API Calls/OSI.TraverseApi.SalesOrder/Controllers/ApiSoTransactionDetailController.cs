...
namespace TRAVERSE.Web.API.SalesOrder.Controllers
{
    ...
    {
        #region Web Methods
        [ApiRoute(FunctionID, 2f, "transaction/{transId}/lineitem/{id:int?}", typeof(TransactionDetailLineItem), new object[] { ApiSoTransactionSerialController.FunctionID, typeof(TransactionSerial), ApiSoTransactionLotController.FunctionId, typeof(TransactionDetailExt) })]
        ...
        protected virtual void ItemIdPropertyChanged(TransactionDetailLineItem entity)
        {
            ...
            if (Utility.INYN)
            {
                ...
                if (entity.InItem == null)
                {
                    entityList = Alias.FindAlias(CompId, entity.ItemId, CurrentTransaction.CustId, ItemAliasType.Customer);
                ...
                if (string.IsNullOrEmpty(entity.CustomerPartNumber))
                {
                    if (entityList == null)
                    {
                        entityList = Alias.FindCustomerItemByAlias(CompId, entity.ItemId, CurrentTransaction.CustId);
