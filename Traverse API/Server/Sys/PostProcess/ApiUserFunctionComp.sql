...
IF NOT EXISTS (SELECT * FROM sys.tables WHERE [object_id] = OBJECT_ID(N''[dbo].[ApiUserFunctionComp]''))
BEGIN
	CREATE TABLE [dbo].[ApiUserFunctionComp] (
		[ID] [bigint] NOT NULL IDENTITY(1, 1),
		[UserFunctionID] [bigint] NULL,			--BL Required; FK:[ApiUserFunction]
		[CompID] [nvarchar](3) NULL,			--BL Required
		...
		[ts] [timestamp] NULL,
		CONSTRAINT [PK_ApiUserFunctionComp] PRIMARY KEY CLUSTERED (
			[ID] ASC
		) ON [PRIMARY]
	) ON [PRIMARY];
END
